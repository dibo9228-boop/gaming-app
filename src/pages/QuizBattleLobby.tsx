...
type InviteWithDetails = Tables<"quiz_battle_invites"> & {
  room?: { id: string; invite_code: string; status: string };
  from_display_name?: string;
};

const LOBBY_PATH = "/game/quiz-battle/lobby";
...
const QuizBattleLobby = () => {
  ...
  const [detailsRoom, setDetailsRoom] = useState<Tables<"quiz_battle_rooms"> | null>(null);
  ...
  const fetchInvites = useCallback(async () => {
    if (!user) return;
    const { data: inviteList } = await supabase.from("quiz_battle_invites").select("*").eq("to_user_id", user.id);
    ...
    const [roomsRes, profilesRes] = await Promise.all([
      supabase.from("quiz_battle_rooms").select("id, invite_code, status").in("id", roomIds),
  ...
  const { run: createRoomAction, loading: creatingRoom } = useApiAction(async () => {
    ...
    navigate(`/game/quiz-battle/multi/${data.id}`);
  });
  ...
  const { run: joinByCodeAction, loading: joiningRoom } = useApiAction(async (code: string) => {
    ...
    const { error: upErr } = await supabase
      .from("quiz_battle_rooms")
      .update({ guest_id: user.id, status: "playing", question_started_at: new Date().toISOString() })
      .eq("id", room.id);
    if (upErr) throw upErr;
    navigate(`/game/quiz-battle/multi/${room.id}`);
  });
  ...
  const { run: deleteRoomAction, loading: deletingRoom } = useApiAction(async (roomId: string) => {
    if (!user) return;
    const { error } = await supabase.from("quiz_battle_rooms").delete().eq("id", roomId).eq("host_id", user.id);
    if (error) throw error;
    await fetchRooms();
  });
  ...
  useEffect(() => {
    ...
    const ch1 = supabase
      ...
      .subscribe();
    const ch2 = supabase
      .channel("quiz-lobby-invites")
      .on("postgres_changes", { event: "*", schema: "public", table: "quiz_battle_invites" }, () => fetchInvites())
      .subscribe();
    return () => {
      supabase.removeChannel(ch1);
      supabase.removeChannel(ch2);
    };
  }, [user, fetchRooms, fetchInvites]);
  ...
  const openDetails = async (room: Tables<"quiz_battle_rooms">) => {
    setDetailsRoom(room);
    setDetailsOpen(true);
              ...
              {invites.map((inv) => (
                <div key={inv.id} className="bg-accent/10 border border-accent/30 rounded-lg p-3 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => supabase.from("quiz_battle_invites").delete().eq("id", inv.id).then(fetchInvites)}>رفض</Button>
                  </div>
                </div>
              ))}
            </div>
                ...
                <div key={room.id} className="bg-card border border-border rounded-lg p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {room.status === "playing" && <Button size="sm" onClick={() => navigate(`/game/quiz-battle/multi/${room.id}`)}>تابع اللعب</Button>}
                    </div>
                  </div>
                  {room.status === "waiting" && (
                    <>
