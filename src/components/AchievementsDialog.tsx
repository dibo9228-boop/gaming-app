...
import { ACHIEVEMENTS, isUnlocked, getUnlockedCount } from "@/lib/achievements";
...
export function AchievementsDialog({
  ...
  const unlockedCount = getUnlockedCount(totalXp);
  const nextAchievement = ACHIEVEMENTS.find((a) => !isUnlocked(a, totalXp));
  const progressToNext = nextAchievement
    ? Math.min(
        100,
        (totalXp / nextAchievement.xpRequired) * 100
      )
    : 100;
      ...
      <DialogContent className="max-w-sm sm:max-w-md" dir="rtl">
        ...
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground font-body">
            {unlockedCount} من {ACHIEVEMENTS.length} مُفتوح — نقاطك: {totalXp}
          </p>
          {nextAchievement && (
          ...
          <div className="max-h-64 overflow-y-auto space-y-2 pr-1">
            {ACHIEVEMENTS.map((a) => {
              const unlocked = isUnlocked(a, totalXp);
              return (
                <div
                  key={a.id}
                  className={`flex items-center gap-3 rounded-lg border p-3 font-body transition-colors ${
                    unlocked
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{a.descriptionAr}</p>
                  </div>
                  {unlocked ? (
